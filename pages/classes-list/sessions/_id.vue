                ...
                <div slot="raw-content" class="table-responsive">
                    <table class="table" :class="tableClass">
                        ...
                        <tbody>
                            ...
                            <td>{{item.start_time_only}}</td>
                            
                            <td
                            v-if="!item.is_expired"
                            >
                                <button  class="btn btn-primary" v-if="item.status == session_status.PendingSession && item.is_today" @click="createJoinUrl(item , index)">Start Session
                                </button>
                                <strong><a  v-if="item.status == session_status.OngoingSession">Session Ongoing</a></strong>
                                <button class="btn btn-primary" v-if="item.status == session_status.PendingSession" type="button" data-toggle="modal" 
                                 data-target="#myModal" @click="session_id = item.id, session_url= item.custom_link, class_index = index">
                                 {{item.custom_link ? 'Update Custom Session URL' : 'Add Custom Session URL'}}</button>
                                <button class="btn btn-primary" v-if="item.custom_link" @click="viewUrl(item.custom_link)">View </button>
                            </td>
                            <td v-else>
                              Expired
                            </td>
...
const tableColumns = ["Start Date" , "Start Time" , "Create Session Link"];
  ...
  methods:{
    async getClases(records , page) {
      ...
      try {
        ...
        let res = await this.$axios.post('/class/organization/class/sessions' , {
          records,
          page,
          class_id: this.$router.currentRoute.params.id
        })
