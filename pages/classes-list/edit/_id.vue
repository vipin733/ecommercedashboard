<template>
     <div id="upper_div" class="row">
    <div class="col-xl-12 col-lg-12 col-md-12">
      <card class="card" title="Update Class">
        <div>
          <form @submit.prevent>
            <div class="row">
              <div class="col-md-6">
            ...
            <div v-if="data.dates.length > 0" :key="dateKey">
              <h2>Session Dates</h2>
              <!-- {{ data.dates }} -->
            ...
            <div class="text-center">
              <p-button type="info" round @click.native.prevent="createClass">
                {{ this.isLoading ? "Updating..." : "Update Class" }}
              </p-button>
            </div>
...
export default {
 ...
 data() {
  ...
  methods: {
    async createClass() {
      try {
        ...
        this.isLoading = true
        let res = await this.$axios.post('/class/update', this.data)
        this.$swal({
          title: "Success",
          icon: 'success',
    ...
    async getClassDetail(id){
      try {
        ...
        let res = await this.$axios.post('/class/detail', {
          class_id: id
        })

         this.data = res.data.data

         if(res.data.data.section_id){

          this.data.section = this.sections.find(element => element.id  ==  res.data.data.section_id)
        } 
        this.data.class_id = id
        // this.data.dates = []
        let dates = res.data.data.dates
        this.data.start_time = this.data._start_time

        // if(dates.length > 0){
        //   dates.forEach(element => {
        //       this.data.dates.push(element)
        //   });
        // }
  ...
  async mounted() {
    await this._getSections()
    this.getClassDetail(this.$router.currentRoute.params.id)
  }
